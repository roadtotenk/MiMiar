...
export default function withCalls<P>(...calls: Call[]): (Component: React.ComponentType<P>) => React.ComponentType<SubtractProps<P, ApiProps>> {
  return (Component: React.ComponentType<P>): React.ComponentType<any> => {
    ...
    return calls.reverse().reduce((Component, call): React.ComponentType<any> => {
      return Array.isArray(call) ? withCall(...call)(Component as unknown as React.ComponentType<ApiProps>) : withCall(call)(Component as unknown as React.ComponentType<ApiProps>);
    }, Component);
  };
}
